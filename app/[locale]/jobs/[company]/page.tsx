...
export async function generateMetadata({ params }: { params: { company: string } }): Promise<Metadata> {
    const name = params.company.split('-').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ');
    return {
...
export default function CompanySpecificJobsPage({ params }: { params: { locale: string; company: string } }) {
