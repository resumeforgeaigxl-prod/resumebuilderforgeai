        ...
        <div className="space-y-10 max-w-5xl mx-auto">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-[#1E2A42] pb-8">
                ...
                <div className="flex items-center gap-4">
                     <Badge variant="outline" className="px-4 py-1.5 rounded-xl border-[#7C5CFC]/20 bg-[#7C5CFC]/5 text-[#7C5CFC] font-bold text-[10px]">
            ...
            {isLoading ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {questions.map((q) => (
                        <Link key={q.id} href={`/${locale}/prepforge/app?slug=${q.slug}`}>
                            <div className="p-6 h-full flex flex-col rounded-xl border border-[#1E2A42] bg-[#0D1220]/60 hover:border-[#7C5CFC]/20 transition-all group">
                                <div className="flex items-start justify-between mb-4">
                                    <div className="p-3 rounded-xl bg-white/5 text-[#7C5CFC] group-hover:scale-105 transition-transform">
                                        <CircuitBoard className="w-5 h-5" />
                                    </div>
                                    <Badge variant="outline" className="px-2.5 py-1 font-bold text-[10px] text-[#00D4A0] border-[#00D4A0]/20 bg-[#00D4A0]/5">
                                        {q.topic}
                                    </Badge>
                                </div>
                                <h3 className="text-base font-bold text-white mb-2 group-hover:text-[#7C5CFC] transition-colors">
                                    {q.title}
                                </h3>
                                ...
                                <div className="mt-auto flex items-center justify-between pt-4 border-t border-[#1E2A42]">
                                    ...
                                    <ChevronRight className="w-3.5 h-3.5 text-[#4A5568] group-hover:text-[#7C5CFC] transition-all" />
                                </div>
                            </div>
                        </Link>
                    ))}
                ...
                <div className="p-12 rounded-2xl flex flex-col items-center text-center space-y-6 bg-[#0D1220]/60 border border-[#1E2A42]">
                    <div className="p-6 rounded-full bg-[#7C5CFC]/10 text-[#7C5CFC]">
                        <Construction className="w-12 h-12" />
                    </div>
                    <div>
                    <h2 className="text-xl font-bold text-white mb-2">No Patterns Yet</h2>
                    <p className="text-sm text-[#7A8BA8] max-w-md mx-auto">
                        Generate some reasoning patterns in the <Link href={`/${locale}/prepforge/app`} className="text-[#7C5CFC] underline underline-offset-4">Logic Engine</Link> to start filling your track.
                    </p>
                    </div>
                </div>
            )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 opacity-30 pointer-events-none grayscale">
                {['Blood Relations', 'Syllogism', 'Seating Arrangement'].map((topic) => (
                    <div key={topic} className="p-6 rounded-xl border border-[#1E2A42] bg-[#0D1220]/60">
                        <div className="p-3 rounded-xl bg-white/5 w-fit mb-4 text-[#4A5568]"><Brain className="w-5 h-5" /></div>
