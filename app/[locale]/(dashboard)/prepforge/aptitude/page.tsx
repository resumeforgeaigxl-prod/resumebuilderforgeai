        ...
        <div className="space-y-12 animate-fade-in max-w-7xl mx-auto">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-white/5 pb-12">
                <div className="space-y-4">
                    ...
                    <h1 className="text-4xl md:text-6xl font-black text-white tracking-tighter leading-none italic">
                        Aptitude <span className="bg-clip-text text-transparent bg-gradient-to-r from-blue-400 to-indigo-400">Hub_</span>
                    </h1>
                ...
                <div className="flex items-center gap-4">
                     <Badge variant="outline" className="px-6 py-2 rounded-xl border-blue-500/20 bg-blue-500/5 text-blue-400 font-black">
                        {questions.length} PATTERNS DEPLOYED
                    </Badge>
                </div>
            </div>
            ...
            {isLoading ? (
                <div className="p-20 flex flex-col items-center justify-center text-center">
                    ...
                    <p className="text-slate-500 font-black uppercase tracking-widest text-[10px]">Syncing with Prep Matrix...</p>
                </div>
            ) : questions.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {questions.map((q) => (
                        <Link key={q.id} href={`/${locale}/prepforge/app?slug=${q.slug}`}>
                            <Card glass className="p-8 h-full flex flex-col border-white/5 bg-white/[0.01] hover:bg-white/[0.03] hover:border-blue-500/30 transition-all duration-300 group">
                                <div className="flex items-start justify-between mb-8">
                                    ...
                                    <Badge variant="outline" className="px-3 py-1 font-black text-[10px] uppercase tracking-widest text-blue-400 border-blue-500/20 bg-blue-500/5">
                                        {q.topic}
                                    </Badge>
                                </div>
                ...
                <Card glass className="p-20 flex flex-col items-center text-center space-y-8 bg-blue-500/[0.02] border-white/5">
                    <div className="p-10 rounded-full bg-blue-500/10 text-blue-400">
                        <Construction className="w-20 h-20" />
                    </div>
                    <div>
                    <h2 className="text-3xl font-black text-white mb-4 tracking-tighter uppercase italic">Protocol Buffer Empty</h2>
                    <p className="text-slate-500 font-medium max-w-xl mx-auto leading-relaxed">
                        Generate some aptitude signals in the <Link href={`/${locale}/prepforge/app`} className="text-blue-400 underline underline-offset-4">Logic Engine</Link> to start filling your track.
                    </p>
                    </div>
                </Card>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 opacity-30 pointer-events-none grayscale">
                {['Time & Work', 'Profit & Loss', 'Percentage'].map((topic) => (
                    <Card key={topic} glass className="p-8 border-white/5 bg-white/[0.01]">
                        <div className="p-4 rounded-xl bg-white/5 w-fit mb-6 text-slate-500"><Divide className="w-6 h-6" /></div>
                        <h3 className="text-xl font-black text-white mb-2 tracking-tighter uppercase italic">{topic}</h3>
                        <p className="text-xs text-slate-500 leading-relaxed mb-6 font-medium">Initializing modules...</p>
