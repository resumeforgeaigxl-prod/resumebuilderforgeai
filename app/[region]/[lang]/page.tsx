      ...
      <main className="relative z-10 pt-32 pb-32">
        <section className="flex flex-col items-center text-center max-w-4xl mx-auto mt-10 mb-32">
          ...
          >
            ...
            >
              {t("build_resume_free")} <ArrowRight className="w-4 h-4" />
            </Link>
            <Link
              href={`/${region}/${locale}/login`}
        ...
        {/* ... (rest of sections keep original content for now, but following same pattern) ... */}
        ...
        <section className="max-w-6xl mx-auto px-6 py-20 mb-20">
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            <div>
              <div className="inline-flex text-blue-400 font-bold tracking-widest text-xs uppercase mb-4">{t('ats_compat')}</div>
              <h2 className="text-4xl font-bold text-white mb-6 leading-tight">{t('ats_match_title')}</h2>
              <p className="text-slate-400 text-lg mb-8">{t('ats_match_desc')}</p>
              <ul className="space-y-4">
                {[
                  t('ats_feature_1') || 'Live keyword matching',
                  t('ats_feature_2') || 'Action verb analysis',
                  t('ats_feature_3') || 'Readability optimization',
                  t('ats_feature_4') || 'Recruiter scan-time estimation'
                ].map((item, i) => (
            ...
            <div className="relative">
              ...
              <div className="relative bg-[#0d1117] border border-white/10 rounded-2xl p-6 shadow-2xl">
                <div className="flex justify-between items-center mb-6">
                  <div>
                    <p className="text-xs text-slate-400 uppercase font-bold tracking-wider">{t('ats_score')}</p>
                    <p className="text-sm text-slate-500">{t('ats_score_subtitle')}</p>
                  </div>
                ...
                <div className="space-y-4">
                  <div>
                    <div className="flex justify-between text-xs mb-1"><span className="text-slate-300">{t('keywords')}</span><span className="text-slate-400">25/27</span></div>
                    <div className="h-2 bg-white/5 rounded-full overflow-hidden"><div className="h-full bg-emerald-500 w-[92%]" /></div>
                  </div>
                  <div>
                    <div className="flex justify-between text-xs mb-1"><span className="text-slate-300">{t('action_verbs')}</span><span className="text-slate-400">85/100</span></div>
                    <div className="h-2 bg-white/5 rounded-full overflow-hidden"><div className="h-full bg-blue-500 w-[85%]" /></div>
                  </div>
                </div>

                <div className="mt-6 pt-6 border-t border-white/5">
                  <p className="text-xs text-red-400 font-bold mb-2">{t('missing_keywords')}:</p>
              ...
              >
                ...
                <div>
                  <p className="text-xs font-bold text-gray-500">{t('improvement')}</p>
        ...
        <section id="how-it-works" className="max-w-7xl mx-auto px-6 py-20 text-center mb-20">
          <h2 className="text-3xl md:text-5xl font-bold text-white mb-16">{t('how_it_works_title')}</h2>
          <div className="grid md:grid-cols-4 gap-8">
            {[
              { icon: <Upload className="w-6 h-6" />, title: t('step_1_title'), desc: t('step_1_desc'), num: "01" },
              { icon: <Target className="w-6 h-6" />, title: t('step_2_title'), desc: t('step_2_desc'), num: "02" },
              { icon: <BrainCircuit className="w-6 h-6" />, title: t('step_3_title'), desc: t('step_3_desc'), num: "03" },
              { icon: <FileText className="w-6 h-6" />, title: t('step_4_title'), desc: t('step_4_desc'), num: "04" },
            ].map((step, i) => (
        ...
        <section id="features" className="max-w-7xl mx-auto px-6 py-20">
          <div className="text-center mb-16">
            <h2 className="text-3xl md:text-5xl font-bold text-white mb-6">{t('features_title')}</h2>
            <p className="text-slate-400 max-w-2xl mx-auto text-lg">{t('features_desc')}</p>
          </div>

          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            <FeatureCard icon={<WandIcon />} title={t('ai_builder')} desc={t('ai_builder_desc')} />
            <FeatureCard icon={<ScanIcon />} title={t('ats_analyzer')} desc={t('ats_analyzer_desc')} />
            <FeatureCard icon={<Target className="w-6 h-6 text-emerald-400" />} title={t('jd_matching')} desc={t('jd_matching_desc')} />
            <FeatureCard icon={<Layout className="w-6 h-6 text-pink-400" />} title={t('portfolio_gen')} desc={t('portfolio_gen_desc')} />
            <FeatureCard icon={<TestIcon />} title={t('interview_prep_title')} desc={t('interview_prep_desc')} />
            <FeatureCard icon={<MessageSquareWarning className="w-6 h-6 text-orange-400" />} title={t('ai_coach') || "AI Coach"} desc={t('ai_coach_desc') || "Career optimization prep."} />
            <FeatureCard icon={<Briefcase className="w-6 h-6 text-cyan-400" />} title={t('job_board') || "Job Board"} desc={t('job_board_desc') || "Curated jobs with direct ATS match scores."} />
            <FeatureCard icon={<Shield className="w-6 h-6 text-yellow-400" />} title={t('secure_auth') || "Secure Auth"} desc={t('secure_auth_desc') || "OAuth logins via Google or GitHub."} />
          </div>
        </section>
        ...
        <section className="max-w-6xl mx-auto px-6 py-20 border-t border-white/5">
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            ...
            <div className="order-1 lg:order-2">
              <div className="inline-flex text-purple-400 font-bold tracking-widest text-xs uppercase mb-4">{t('interview_prep')}</div>
              <h2 className="text-4xl font-bold text-white mb-6 leading-tight">{t('interview_prep_title')}</h2>
              <p className="text-slate-400 text-lg mb-8">{t('interview_prep_desc')}</p>
              <Link href={`/${region}/${locale}/signup`} className="text-white font-medium hover:text-purple-400 flex items-center gap-2 transition-colors">{t('start_mock_test')} <ArrowRight className="w-4 h-4" /></Link>
            </div>
          </div>
        </section>
        ...
        <section className="max-w-7xl mx-auto px-6 py-20 bg-black/30 border-y border-white/5 my-20">
          <div className="text-center mb-12">
            <h2 className="text-3xl md:text-5xl font-bold text-white mb-6">{t('portfolio_title') || "Web Portfolio"}</h2>
            <p className="text-slate-400 max-w-2xl mx-auto text-lg mb-8">{t('portfolio_subtitle') || "One click turns your resume into a stunning website."}</p>
        ...
        <section className="max-w-4xl mx-auto px-6 pt-20 pb-10 text-center">
          <div className="p-12 md:p-16 rounded-[2.5rem] bg-gradient-to-br from-purple-900/50 to-blue-900/50 border border-purple-500/30 relative overflow-hidden">
            ...
            <h2 className="text-4xl md:text-5xl font-bold text-white mb-6 relative z-10">{t('ready_to_start')}</h2>
            <p className="text-lg text-blue-200 mb-10 relative z-10 max-w-xl mx-auto">{t('ready_desc')}</p>
            <Link href={`/${region}/${locale}/signup`} className="inline-flex items-center justify-center gap-2 px-10 py-5 rounded-full bg-white text-black font-bold hover:bg-slate-200 hover:scale-105 transition-all shadow-[0_0_40px_rgba(255,255,255,0.2)] relative z-10">
              {t('get_started_free')} <ArrowRight className="w-5 h-5" />
            </Link>
          </div>
        </section>
