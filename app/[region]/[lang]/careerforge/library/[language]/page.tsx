...
export default function LanguageTopicListPage() {
    const params = useParams() as { region: string; lang: string; language: string };
    const { region, lang, language } = params;
    ...
    return (
        <div className="max-w-5xl mx-auto pt-28 pb-24 px-6 space-y-10">
            <Link
                href={`/${region}/${lang}/careerforge/library`}
                ...
                {topics.length === 0 ? (
                    ...
                    <div className="space-y-2">
                        {topics.map((topic) => (
                            <Link
                                key={topic.id}
                                href={`/${region}/${lang}/careerforge/library/${language}/${topic.slug}`}
