...
import { Loader2, Send, Bot, User, AlertCircle, ShieldCheck, Copy, Share2, Check, Terminal } from 'lucide-react';
...
export default function JobForgeAIPage() {
    ...
    const shareConversation = () => {
        ...
        const shareUrl = `${window.location.origin}/jobforgeai/share/${conversationId}`;
    ...
    return (
        <div className="min-h-screen bg-[#020205] text-slate-200 flex flex-col selection:bg-indigo-500/30">
            ...
            <header className="sticky top-0 z-30 bg-[#020205]/80 backdrop-blur-2xl border-b border-white/5 px-6 py-4">
                <div className="max-w-5xl mx-auto flex items-center justify-between">
                    <div className="flex items-center gap-6">
                        <Link href="/dashboard" className="p-2.5 bg-white/5 hover:bg-white/10 rounded-2xl transition-all border border-white/5 group">
            ...
            {/* Chat Area */}
            <main className="flex-1 max-w-5xl w-full mx-auto p-6 md:p-10 flex flex-col gap-10 overflow-y-auto mb-28 scroll-smooth custom-scrollbar">
                {blocked && (
                                ...
                                <div className="space-y-2">
                                    <div className={`flex items-center gap-2 mb-1 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                                        <span className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">
                                            {msg.role === 'user' ? (userName || 'Candidate') : (msg.role === 'system' ? 'Terminal Error' : 'JobForgeAI')}
                                        </span>
                                    </div>
                                    <div className={`p-6 rounded-[2rem] text-sm md:text-base ${msg.role === 'user' ? 'bg-indigo-600 text-white rounded-tr-none shadow-2xl shadow-indigo-500/5' :
                                        msg.role === 'system' ? 'bg-red-500/20 border border-red-500/30 text-red-200' :
                                            'bg-white/[0.03] border border-white/5 text-slate-200 rounded-tl-none whitespace-pre-wrap leading-relaxed shadow-lg ring-1 ring-white/5 relative group-hover:bg-white/[0.05] transition-all'
                                        }`}>
                                        {msg.content}
                                        {msg.role === 'assistant' && (
                                            <button
                        ...
                        {chatLoading && (
                            <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0 }} className="flex gap-4 mr-auto max-w-[85%]">
                                ...
                                <div className="p-6 rounded-[2rem] rounded-tl-none bg-white/[0.03] border border-white/5 text-slate-500 italic text-sm font-mono flex items-center gap-3">
                                    Processing cryptographic career insights...
                                </div>
                            </motion.div>
                        )}
                    </AnimatePresence>
            ...
            {/* Input Area */}
            <div className="fixed bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-[#020205] via-[#020205] to-transparent pointer-events-none pb-10">
                <form onSubmit={handleSendMessage} className="max-w-5xl mx-auto relative pointer-events-auto group">
                    <input
                        type="text"
                        value={input}
                        onChange={(e) => setInput(e.target.value)}
                        placeholder="Ask about technical interviews, JD matching, or resume optimization..."
                    ...
                    <button
                        ...
                        className="absolute right-3 top-3 bottom-3 aspect-square bg-indigo-600 hover:bg-indigo-500 text-white rounded-full flex items-center justify-center transition-all disabled:opacity-50 disabled:hover:bg-indigo-600 shadow-xl shadow-indigo-500/20 active:scale-95"
                    >
                        {chatLoading ? <Loader2 className="w-5 h-5 animate-spin" /> : <Send className="w-5 h-5" />}
                    </button>
                    {blocked && <p className="text-center text-[10px] font-black uppercase tracking-[0.2em] text-red-500 mt-4 animate-pulse">Neural Link Severed: Security Protocol Violation</p>}
                </form>
            </div>
        </div>
    );
