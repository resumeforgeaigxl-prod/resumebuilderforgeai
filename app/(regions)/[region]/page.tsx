...
import {
    ArrowRight, FileText, Sparkles, Zap, Shield, Target, Layout,
    Upload, CheckCircle2, Lock, TrendingUp, XCircle, BrainCircuit,
    Briefcase, MessageSquareWarning, Globe,
} from 'lucide-react';
...
interface Props { params: { region: string } }

export default function RegionalPage({ params }: Props) {
    const r = params.region;
    const region = REGION_DATA[r];
    if (!region) notFound();

    const { name, flag, free, pro, premium, career } = region;
            ...
            <main className="relative z-10 pt-40 pb-32">
                ...
                <section className="flex flex-col items-center text-center max-w-4xl mx-auto mt-10 mb-32 px-6">
                    ...
                    >
                        ...
                        <span className="text-sm font-medium text-gray-300">{flag} {name} Edition · Powered by advanced AI</span>
                    </motion.div>
                    ...
                    >
                        <Link href="/signup" className="flex items-center justify-center gap-2 px-8 py-4 rounded-full bg-white text-black font-semibold hover:bg-gray-100 hover:scale-105 transition-all w-full sm:w-auto shadow-[0_0_40px_rgba(255,255,255,0.3)]">
                            Build Resume Free <ArrowRight className="w-4 h-4" />
                        </Link>
                        <Link href="/login" className="flex items-center justify-center gap-2 px-8 py-4 rounded-full bg-white/5 border border-white/10 text-white font-semibold hover:bg-white/10 transition-colors w-full sm:w-auto">
                            View Dashboard
                        </Link>
                    </motion.div>
                </section>
                ...
                <section className="max-w-7xl mx-auto px-6 py-24 mb-20 border-y border-white/5 bg-black/20">
                    ...
                    <div className="bg-gradient-to-br from-purple-500/10 to-blue-500/10 border border-purple-500/20 p-8 md:p-12 rounded-3xl text-center max-w-4xl mx-auto">
                ...
                <section id="features" className="max-w-7xl mx-auto px-6 py-20">
                    ...
                    <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
                        <FeatureCard icon={<Sparkles className="w-6 h-6 text-purple-400" />} title="AI Builder" desc="FAANG-mode bullet enhancer that rewrites history for maximum impact." />
                        <FeatureCard icon={<Zap className="w-6 h-6 text-blue-400" />} title="ATS Analyzer" desc="Real-time grading for action verbs, missing metrics, and readability." />
                        ...
                        <FeatureCard icon={<BrainCircuit className="w-6 h-6 text-orange-400" />} title="AI Mock Tests" desc="Generate technical interview questions specific to your exact resume." />
                ...
                {/* 9. Pricing — region-specific prices */}
                <section id="pricing" className="max-w-7xl mx-auto px-6 py-20">
                    <div className="text-center mb-16">
                        <h2 className="text-3xl md:text-5xl font-bold text-white mb-6">
                            Simple, transparent pricing {flag}
                        </h2>
                        <p className="text-slate-400 max-w-2xl mx-auto text-lg">Invest in your career today. Cancel anytime. All prices shown in local {name} currency.</p>
                    </div>
                    ...
                    <div className="grid sm:grid-cols-2 xl:grid-cols-4 gap-6 items-stretch">
                        ...
                        <div className="p-7 rounded-3xl bg-gradient-to-b from-blue-900/40 to-[#070710] border border-blue-500/50 relative shadow-[0_0_30px_rgba(59,130,246,0.2)] flex flex-col">
                            ...
                            <div className="mb-1 flex items-baseline gap-1"><span className="text-4xl font-bold text-white">{pro}</span><span className="text-slate-400 text-sm">/one-time</span></div>
                        ...
                        <div className="p-7 rounded-3xl bg-gradient-to-b from-purple-900/30 to-[#070710] border border-purple-500/40 flex flex-col">
                            ...
                            <div className="mb-1 flex items-baseline gap-1"><span className="text-4xl font-bold text-white">{premium}</span><span className="text-slate-400 text-sm">/month</span></div>
                        ...
                        <div className="p-7 rounded-3xl bg-gradient-to-b from-amber-900/30 to-[#070710] border border-amber-500/40 flex flex-col">
                            ...
                            <div className="mb-1 flex items-baseline gap-1"><span className="text-4xl font-bold text-white">{career}</span><span className="text-slate-400 text-sm">/month</span></div>
...
function FeatureCard({ icon, title, desc }: { icon: React.ReactNode; title: string; desc: string }) {
    return (
        <div className="p-6 rounded-2xl bg-white/[0.03] border border-white/5 hover:bg-white/[0.06] hover:border-white/10 transition-all group">
            <div className="w-12 h-12 rounded-xl bg-white/5 flex items-center justify-center mb-5 group-hover:scale-110 transition-transform">{icon}</div>
